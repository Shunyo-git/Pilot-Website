...
namespace Pilot.Profile
{
    public sealed class CustomProfileProvider : ProfileProvider
    {

        // Get an instance of the Profile DAL using the ProfileDALFactory
        //private static readonly IFSonlineProfileProvider dal = DataAccess.CreateFSonlineProfileProvider();

        private static readonly IProfileProvider dal = DataAccessHelper.GetProfileDataAccess();
        // Private members
        private const string ERR_INVALID_PARAMETER = "Invalid Profile parameter:";
        private const string PROFILE_ACCOUNT = "MemberInfo";
        private static string applicationName = "Pilot 1.0";
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            if (string.IsNullOrEmpty(config["description"]))
            {
                ...
                config.Add("description", "Pilot Custom Profile Provider");
            }

            if (string.IsNullOrEmpty(name))
                name = "PilotProfileProvider";
            ...
            foreach (SettingsProperty prop in collection)
            {
                ...
                switch (pv.Property.Name)
                {
                    ...
                    case PROFILE_ACCOUNT:
                        if (isAuthenticated)
                            pv.PropertyValue = GetMemberInfo(username);
                        break;
                    default:
            ...
            foreach (SettingsPropertyValue pv in collection)
            {
                ...
                if (pv.PropertyValue != null)
                {
                    switch (pv.Property.Name)
                    {
                        ...
                        case PROFILE_ACCOUNT:
                            if (isAuthenticated)
                                SetMemberInfo(uniqueID, (MemberInfo)pv.PropertyValue);
                            break;
                        default:
                            throw new ApplicationException(ERR_INVALID_PARAMETER + " name.");
                    }
        ...
        private static MemberInfo GetMemberInfo(string username)
        {
            return dal.GetMemberInfo(username, applicationName);
        }


        // Profile setters
        // Update account info
        private static void SetMemberInfo(int uniqueID, MemberInfo addressInfo)
        {
            dal.SetMemberInfo(uniqueID, addressInfo);
        }
